...
def main():
    ...
    with open('result/{}_{}_{}_{}.csv'.format(args.low, args.high, args.path.split('/')[-1][:-7], use_y), 'w',
        ...
        for _, (val_data, id) in enumerate(test_loader):
            ...
            x0 = val_data
            ####set input data and target data corrsponding to the input resolution and output resolution
            deg_xt, mask_l, y, condition, xs, mask_s = sde.get_deg_t_s(x0, time_l, time_s)
            ...
            re_sde_plus = util.tensor2mriimg(visuals["Re"].squeeze())
            ...
            re_s = util.get_xs_s_mean(visuals["Re"].unsqueeze(0), mask_s)
